...
class ServerManager:
    ...
    def start(self):
        ...
        for i in range(50):  # 최대 5초 대기
            time.sleep(0.1)
            try:
                response = requests.get(f"http://localhost:{self.port}/api/game/state", timeout=1)
...
class GameLauncher(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.server_status_label = ttk.Label(server_top_frame, text="⏹️ 서버 중지됨", font=("맑은 고딕", 11))
        self.server_status_label.pack(side=tk.LEFT)
        
        self.open_browser_btn = ttk.Button(server_top_frame, text="🌐 브라우저에서 게임 열기", command=self.open_browser, state=tk.DISABLED)
        self.open_browser_btn.pack(side=tk.RIGHT, padx=(10, 0))
        
        self.stop_server_btn = ttk.Button(server_top_frame, text="⏹️ 서버 중지", command=self.stop_server, state=tk.DISABLED)
        self.stop_server_btn.pack(side=tk.RIGHT, padx=(5, 0))
        
        self.start_server_btn = ttk.Button(server_top_frame, text="▶️ 서버 시작", command=self.start_server)
        self.start_server_btn.pack(side=tk.RIGHT)
        
        # === CSV 파일 선택 섹션 ===
        csv_frame = ttk.LabelFrame(main_frame, text="📂 노래 목록 (CSV) 선택", padding="10")
        ...
        # CSV 파일 드롭다운
        self.csv_combo = ttk.Combobox(csv_select_frame, textvariable=self.current_csv, state="readonly", width=40)
        self.csv_combo.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.refresh_csv_list()
        
        ttk.Button(csv_select_frame, text="🔄 새로고침", command=self.refresh_csv_list, width=10).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(csv_select_frame, text="📁 파일 찾기...", command=self.browse_csv, width=12).pack(side=tk.LEFT, padx=(5, 0))
        
        # CSV 로드 버튼
        load_frame = ttk.Frame(csv_frame)
        load_frame.pack(fill=tk.X, pady=(10, 0))
        
        self.load_csv_btn = ttk.Button(load_frame, text="📥 선택한 CSV 로드", command=self.load_csv)
        ...
        # === 참가자 목록 섹션 ===
        participants_frame = ttk.LabelFrame(main_frame, text="👥 참가자 목록 및 점수", padding="10")
        participants_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # 테이블 (Treeview)
        columns = ("rank", "username", "score")
        self.participants_tree = ttk.Treeview(participants_frame, columns=columns, show="headings", height=15)
        ...
        # 스크롤바
        scrollbar = ttk.Scrollbar(participants_frame, orient=tk.VERTICAL, command=self.participants_tree.yview)
        ...
        # 참가자 수 표시
        self.participant_count_label = ttk.Label(main_frame, text="총 참가자: 0명", font=("맑은 고딕", 10))
        ...
        self.game_status_label = ttk.Label(game_frame, text="게임 대기 중", font=("맑은 고딕", 11))
        self.game_status_label.pack(side=tk.LEFT)
        
        self.reset_btn = ttk.Button(game_frame, text="🔄 점수 초기화", command=self.reset_scores)
        ...
        ttk.Button(log_control_frame, text="🗑️ 로그 지우기", command=self.clear_log).pack(side=tk.RIGHT)
    ...
    def load_csv(self):
        ...
        if not self.server_manager.is_running:
            messagebox.showwarning("경고", "서버가 실행 중이 아닙니다. 먼저 서버를 시작해주세요.")
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.csv_status_label.config(text=f"✅ {count}곡 로드됨", foreground="green")
                logger.info(f"CSV 로드 성공: {count}곡")
            else:
                self.csv_status_label.config(text=f"❌ 로드 실패", foreground="red")
                logger.error(f"CSV 로드 실패: {response.text}")
                messagebox.showerror("오류", f"CSV 로드 실패: {response.text}")
        except Exception as e:
            self.csv_status_label.config(text=f"❌ 오류 발생", foreground="red")
    ...
    def start_server(self):
        ...
        def _start():
            ...
            if success:
                self.server_status_label.config(text="✅ 서버 실행 중 (http://localhost:8000)", foreground="green")
                ...
                try:
                    ...
                    if response.status_code == 200 and "<!DOCTYPE html>" in response.text[:100]:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        songs = response.json()
                        self.csv_status_label.config(text=f"✅ {len(songs)}곡 로드됨", foreground="green")
                        logger.info(f"기본 CSV 로드됨: {len(songs)}곡")
                except Exception as e:
                    logger.debug(f"기본 CSV 상태 확인 실패: {e}")
            else:
                self.server_status_label.config(text="❌ 서버 시작 실패", foreground="red")
                self.start_server_btn.config(state=tk.NORMAL)
                logger.error("서버 시작 실패")
                messagebox.showerror("오류", "서버를 시작할 수 없습니다. 포트 8000이 이미 사용 중일 수 있습니다.")
    ...
    def update_participants_periodically(self):
        ...
        def _fetch_and_update():
            try:
                ...
                # 게임 상태
                state_response = requests.get(f"{self.api_base}/api/game/state", timeout=1)
    ...
    def _update_participants_ui(self, players, state):
        ...
        try:
            ...
            for idx, player in enumerate(players, 1):
                rank_text = f"🥇 {idx}" if idx == 1 else (f"🥈 {idx}" if idx == 2 else (f"🥉 {idx}" if idx == 3 else str(idx)))
                self.participants_tree.insert("", tk.END, values=(rank_text, player["username"], f"{player['score']}점"))
            ...
            if state.get("is_playing"):
                ...
                self.game_status_label.config(text=f"🎵 게임 진행 중 ({progress}/{total}곡)", foreground="blue")
            else:
    ...
    def reset_scores(self):
        ...
        if messagebox.askyesno("확인", "모든 참가자의 점수를 초기화하시겠습니까?"):
            ...
            def _reset():
                try:
                    response = requests.post(f"{self.api_base}/api/game/reset-scores", timeout=5)
                    if response.status_code == 200:
                        # UI 업데이트는 메인 스레드에서
                        self.after(0, lambda: self._update_participants_ui([], {}))
                        self.after(0, lambda: logger.info("점수 초기화 완료"))
                        self.after(0, lambda: messagebox.showinfo("완료", "점수가 초기화되었습니다."))
                    else:
                        self.after(0, lambda: logger.error(f"점수 초기화 실패: {response.text}"))
                        self.after(0, lambda: messagebox.showerror("오류", "점수 초기화 실패"))
                except Exception as e:
                    self.after(0, lambda err=e: logger.error(f"점수 초기화 오류: {err}"))
                    self.after(0, lambda err=e: messagebox.showerror("오류", f"서버 연결 실패: {err}"))
    ...
    def on_closing(self):
        ...
        if self.server_manager.is_running:
            if messagebox.askokcancel("종료", "게임 서버를 종료하고 프로그램을 닫으시겠습니까?"):
